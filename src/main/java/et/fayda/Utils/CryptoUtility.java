...
public class CryptoUtility {
	...
	public static Map<String, String>  encrypt(PublicKey publicKey, String data, String transactionId) {
		...
		try {
			...
			CryptoCore core = new CryptoCore();
			SecretKey secretKey = getSymmetricKey();
			final byte[] encryptedData = symmetricEncrypt(secretKey, dataBytes, ivBytes, aadBytes);			
			final byte[] encryptedSymmetricKey = core.asymmetricEncrypt(publicKey,secretKey.getEncoded()); //asymmetricEncrypt(publicKey, secretKey.getEncoded());

			result.put("ENC_SESSION_KEY",	Base64.encodeBase64URLSafeString(encryptedSymmetricKey));
			result.put("ENC_DATA", java.util.Base64.getUrlEncoder().encodeToString(encryptedData));

			result.put("TIMESTAMP", timestamp);

			result.put("HMAC",Base64.encodeBase64String(symmetricEncrypt(secretKey,HMACUtils.generateHash(data.getBytes(StandardCharsets.UTF_8)),ivBytes,aadBytes)));
