...
public class CryptoCore implements CryptoCoreSpec<byte[], byte[], SecretKey, PublicKey, PrivateKey, String> {
	...
	private String signAlgorithm="RS256";
...
//	  SoftHSM does not support it.Will be removed after HSM implementation
	...
	public byte[] asymmetricDecrypt(PrivateKey privateKey, PublicKey publicKey, byte[] data) {
		BigInteger keyModulus = Objects.nonNull(publicKey) ? ((RSAPublicKey)publicKey).getModulus() : ((RSAPrivateKey)privateKey).getModulus();
		return this.asymmetricDecrypt(privateKey, keyModulus, data);
	}
	...
	private byte[] asymmetricDecrypt(PrivateKey privateKey, BigInteger keyModulus, byte[] data) {
		...
		} catch (NoSuchPaddingException | java.security.NoSuchAlgorithmException var8) {
			throw new NoSuchAlgorithmException(SecurityExceptionCodeConstant.MOSIP_NO_SUCH_ALGORITHM_EXCEPTION.getErrorCode(), SecurityExceptionCodeConstant.MOSIP_NO_SUCH_ALGORITHM_EXCEPTION.getErrorMessage(), var8);
		}

		try {
			cipher.init(2, privateKey);
		} catch (java.security.InvalidKeyException var7) {
			throw new InvalidKeyException(SecurityExceptionCodeConstant.MOSIP_INVALID_KEY_EXCEPTION.getErrorCode(), var7.getMessage(), var7);
		}
		...
		return this.unpadOAEPPadding(paddedPlainText, keyModulus);
	}
	...
	private byte[] unpadOAEPPadding(byte[] paddedPlainText, BigInteger keyModulus) {
		try {
			...
			throw new InvalidKeyException(SecurityExceptionCodeConstant.MOSIP_INVALID_KEY_EXCEPTION.getErrorCode(), var6.getMessage(), var6);
		}
	}
